...
public class SocialSimFlowTest {
	 ...
	 * @param totalSimRuns
	 */
	private static void runSim(String[] args, int kSize, int snapshotInterval,
			int totalSimRuns) {
		for (int i = 0; i < totalSimRuns; i++) {
			SocialSim sim = new SocialSim(SEED);
			List<Graph<Agent, FriendLink>> graphList = sim.runSim(args,
					snapshotInterval);
	 ...
	 * @author biggie
	 */
	private static TimeLineList<Agent> findCommunities(
			List<Graph<Agent, FriendLink>> graphList_, int kSize_) {
		TimeLineList<Agent> evolution = new TimeLineList<Agent>(
				graphList_.size());
		int snapshot = 1;
	 ...
	 * @param evolution
	 */
	private static <V, E> void paintHyperGraph(List<Graph<V, E>> graphEvol,
			TimeLineList<V> evolution) {
		JFrame frame = new JFrame("Simple Graph View");
		frame.setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE);

		Hypergraph<V, Integer> hg = populateHg(graphEvol.get(2),
				evolution.getSnapshot(2));
		HypergraphLayout<V, Integer> l = new HypergraphLayout<V, Integer>(hg,
				CircleLayout.class);

		VisualizationViewer<V, Integer> v = new VisualizationViewer<V, Integer>(
				l, new Dimension(600, 600));
	 ...
	 * @author biggie
	 */
	private static <V, E> Collection<Set<V>> findCommunities(Graph<V, E> graph,
			int k_) {
	 ...
	 * @param <E>
	 */
	private static <V, E> Hypergraph<V, Integer> populateHg(Graph<V, E> graph_,
			List<Community<V>> list_) {
